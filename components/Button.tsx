const Button = ({ link, text, handler, fill }) => {
  const btnStyle = `${
    fill
      ? "dark:text-black dark:bg-white bg-brand-bg text-white hover:bg-secondary hover:border-secondary hover:text-white dark:border-white border-brand-bg"
      : "dark:text-white text-black hover:text-white dark:hover:bg-white hover:bg-brand-bg dark:border-white border-brand-bg"
  } ${
    link ? "inline-block" : ""
